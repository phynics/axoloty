...
open class CoatyObject: Codable {
    ...
    static func addCoreTypeKeys<T>(decoder: Decoder, coreTypeKeys: T.Type)
        where T: CodingKey, T: CaseIterable, T: RawRepresentable, T.RawValue == String {
            guard let allCoreTypeKeys = decoder.currentContext(forKey: "coreTypeKeys") as? CoreTypeKeysContext else {
                return
            }
            coreTypeKeys.allCases.forEach({ key in allCoreTypeKeys.insert(key.rawValue) })
    }

    private func decodeCustomKeys(_ decoder: Decoder) throws {
        guard let coreTypeKeys = decoder.currentContext(forKey: "coreTypeKeys") as? CoreTypeKeysContext else {
            return
        }
