...
public enum AxolotyError: Throwable, Catching {
    ...
    /// A transport-level network error caught at an Axoloty API boundary.
    ///
    /// The associated `error` is the original foreign error (e.g. from
    /// MQTT-NIO or NIO), preserved so `ErrorKit.errorChainDescription(for:)`
    /// can surface the full cause chain. The `reason` describes the context
    /// in which the error was caught.
    case network(error: Error, reason: String)
