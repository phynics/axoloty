...
extension CommunicationManager {
    ...
    func _observeIoState(ioPointId: CoatyUUID) -> BehaviorSubject<IoStateEvent> {
        if let item = self.observedIoStateItems[ioPointId.string] {
            ...
            if let srcItems = self.ioSourceItems[ioPointId.string] {
                ...
                self.ioActorItems.forEach { _, sourceItems in
                    if sourceItems[ioPointId.string] != nil {
                        hasAssociations = true
    ...
    public func observeIoValue(ioActor: IoActor) -> Observable<Any> {
        ...
        var item = self.observedIoValueItems[ioActorId.string]
        if item == nil {
        ...
        if self.ioValueObservable == nil {
            ...
            self.ioValueObservable!
                .subscribe(onNext: { topic, payload in
                    ...
                    if let items = self.ioActorItems[topic] {
                        items.forEach { actorIdString, _ in
                            let actorId = CoatyUUID(uuidString: actorIdString)!
                            if let ioValueItem = self.observedIoValueItems[actorId.string] {
