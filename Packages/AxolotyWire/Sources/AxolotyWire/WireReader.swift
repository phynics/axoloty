...
public struct WireReader {
    ...
    private struct FieldDestination: JSONTokenizerDestination {
        ...
        mutating func objectEndFound(_ token: JSONToken.ObjectEnd, context: consuming Context) { depth -= 1; if depth == 1, let key = context.key { index.append(FieldSlot(key: key, value: context.start..<token.end.byteOffset, content: context.start..<token.end.byteOffset, kind: .object), bytes: bytes) }; if depth == 0 { index.completeValue = true; expectingKey = false } }
        ...
        mutating func arrayEndFound(_ token: JSONToken.ArrayEnd, context: consuming Context) { depth -= 1; if depth == 1, let key = context.key { index.append(FieldSlot(key: key, value: context.start..<token.end.byteOffset, content: context.start..<token.end.byteOffset, kind: .array), bytes: bytes) }; if depth == 0 { index.completeValue = true } }
