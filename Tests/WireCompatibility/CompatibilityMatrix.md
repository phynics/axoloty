...
Identity lifecycle / last will has five executable live scenarios of eleven, evidenced by `Tests/WireCompatibility/Lifecycle/Live/run-lifecycle-matrix.sh`. Three (`unexpected-disconnect-last-will`, `qos-0`, `graceful-deadvertise`) have both subject and observer as CoatyJS reference agents, so they are not cross-implementation evidence for either directional column. The other two, `duplicate-reply` and `late-reply`, are genuine `Modern → JS` evidence: Axoloty is the live Call/Return initiator against pinned CoatyJS 2.4.0 as a (deliberately misbehaving, for these two scenarios) responder — see `Tests/WireCompatibility/Lifecycle/AxolotyLifecycleSubjectTests.swift` and `Tests/WireCompatibility/Reverse/coatyjs-core-consumer.js`. Both were verified end-to-end natively (no `docker`/`podman` on this host) via `Tests/WireCompatibility/Lifecycle/Live/run-lifecycle-call-return.sh`, cross-referencing an independent MQTT capture against the Axoloty application log, not merely a process exit code. See `Tests/WireCompatibility/Lifecycle/Live/README.md` for the full disposition of every catalog scenario. The remaining four lifecycle scenarios (`offline-queueing`, `reconnect-resubscribe`, `broker-restart`, `clean-session`) are recorded as `unsupported`: they need a TCP-level network-manipulation harness (severing/restoring connectivity between Axoloty and the broker) that was not built in this pass — Axoloty itself is now a proven live subject, so this is a harness gap, not an unavailable-subject gap. `qos-1`/`qos-2` remain `unsupported` for a separate, verified reason: pinned `@coaty/core@2.4.0` hardcodes QoS 0 for every publish regardless of configuration.
