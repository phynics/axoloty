...
`Modern → JS` for the seven core capabilities is backed by `Tests/WireCompatibility/Reverse/`: Axoloty produces (or, for request/reply pairs, requests) against the pinned CoatyJS 2.4.0 reference agent, and the test asserts CoatyJS's decoded response semantics, not just wire delivery. It is marked "with normalization" because dynamic identifiers and timestamps are normalized before comparison. `JS → modern` has no live coverage yet: `Tests/WireCompatibility/Live/run-coatyjs-core.sh` only validates the CoatyJS-to-CoatyJS reference wire protocol and has no Axoloty consumer in the loop.
