...
Identity lifecycle / last will has three executable live scenarios of eleven (`unexpected-disconnect-last-will`, `qos-0`, `graceful-deadvertise`, all evidenced by `Tests/WireCompatibility/Lifecycle/Live/run-lifecycle-matrix.sh`), but their subject and observer are both CoatyJS reference agents, so none of them are yet cross-implementation evidence for either directional column; see that directory's README for the full disposition of every catalog scenario. The remaining eight lifecycle scenarios are recorded as `unsupported` (documented reference-fixture or harness limitation), never as passing — notably `qos-1`/`qos-2`, for which a live attempt confirmed pinned `@coaty/core@2.4.0` hardcodes QoS 0 for every publish regardless of configuration.
