...
checker="$root/Tests/Support/check-g6-public-products.sh"
tmp=$(mktemp -d)
trap 'rm -rf "$tmp"' EXIT

fixture="$tmp/repository"
mkdir -p "$fixture/Tests/Support"
cp "$checker" "$fixture/Tests/Support/check-g6-public-products.sh"
cp "$root/Tests/Support/validate-g6-products.mjs" "$fixture/Tests/Support/validate-g6-products.mjs"
...
EOF
(cd "$fixture" && Tests/Support/check-g6-public-products.sh) >/dev/null
sed -i 's/\.executable(name: "axoloty-mcp")/.executable(name: "unexpected")/' "$fixture/Package.swift"
if (cd "$fixture" && Tests/Support/check-g6-public-products.sh) >/dev/null 2>&1; then
