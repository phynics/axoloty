...
checker="$root/Tests/Support/check-g6-product-boundary.sh"
tmp=$(mktemp -d)
trap 'rm -rf "$tmp"' EXIT

fixture="$tmp/repository"
mkdir -p "$fixture/Source/Runtime" "$fixture/Packages/AxolotyStaticRuntime/Sources" "$fixture/Tests/Support"
cp "$checker" "$fixture/Tests/Support/check-g6-product-boundary.sh"
printf '%s\n' \
    'let package = Package(' \
    '    targets: [' \
    '        .target(' \
...
(cd "$fixture" && Tests/Support/check-g6-product-boundary.sh) >/dev/null

printf '%s\n' 'import MQTTNIOClient' > "$fixture/Source/Runtime/Current.swift"
if (cd "$fixture" && Tests/Support/check-g6-product-boundary.sh) >/dev/null 2>&1; then
