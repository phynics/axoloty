...
checker="$root/Tests/Support/check-no-escaping-borrows.sh"
...
mkdir -p "$fixture/Source/Runtime" "$fixture/Tests/Support"
cp "$checker" "$fixture/Tests/Support/check-no-escaping-borrows.sh"
cp "$scanner" "$fixture/Tests/Support/detect-escaping-borrow.pl"
...
if ! (cd "$fixture" && "$fixture/Tests/Support/check-no-escaping-borrows.sh") >/dev/null; then
...
if (cd "$fixture" && "$fixture/Tests/Support/check-no-escaping-borrows.sh") >/dev/null 2>&1; then
    echo "error: checker accepted the known escaping-TopicView shape (6172052)" >&2
    exit 1
fi

if ! (cd "$fixture" && "$fixture/Tests/Support/check-no-escaping-borrows.sh") 2>&1 \
