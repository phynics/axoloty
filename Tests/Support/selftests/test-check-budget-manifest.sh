...
neg() {
    description=$1
    file=$2
    if ! "$SCRIPT_DIR/../checks/check-budget-manifest.sh" "$file" >/dev/null 2>&1; then
        echo "  PASS: $description"
        pass=$((pass + 1))
    else
...
check "check-budget-manifest.sh passes sh -n" sh -n "$SCRIPT_DIR/../checks/check-budget-manifest.sh"
check "budget-manifest.json exists" test -f "$MANIFEST"
check "manifest validates" "$SCRIPT_DIR/../checks/check-budget-manifest.sh" "$MANIFEST"
node "$SCRIPT_DIR/../benchmarks/budget-test-fixtures.mjs" "$MANIFEST" "$TMP"
check "approved-base fixture validates" "$SCRIPT_DIR/../checks/check-budget-manifest.sh" "$TMP/approved-base.json"
