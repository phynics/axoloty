...
node "$root/Tests/Support/evidence/validate-g6-wire-matrix.mjs" "$tmp/matrix.json" "" "$root" >/dev/null
...
if node "$root/Tests/Support/evidence/validate-g6-wire-matrix.mjs" "$tmp/matrix.json" "" "$root" >/dev/null 2>&1; then
