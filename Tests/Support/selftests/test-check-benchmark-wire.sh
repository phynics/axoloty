...
oracle_fingerprint=$(node "$root/Tests/Support/benchmarks/corpus-fingerprint.mjs" "$root/Benchmarks/Corpus")
...
mutated_oracle=$(node "$root/Tests/Support/benchmarks/corpus-fingerprint.mjs" "$mutated_corpus")
...
import { percentile, mad, compare } from "./Tests/Support/benchmarks/benchmark-wire.mjs";
...
JS
sh -n "$script_dir/../checks/check-benchmark-wire.sh"
