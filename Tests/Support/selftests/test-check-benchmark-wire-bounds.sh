...
check "check-benchmark-wire-bounds.sh passes sh -n" sh -n "$SCRIPT_DIR/../checks/check-benchmark-wire-bounds.sh"

check "benchmark enforces the single-tokenizer implementation guard" \
    env AXOLOTY_WIRE_BOUNDS_SOURCE_ONLY=1 sh "$SCRIPT_DIR/../checks/check-benchmark-wire-bounds.sh"
