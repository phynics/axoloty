...
AXOLOTY_WIFI_SSID='ssid ü;${x}' AXOLOTY_WIFI_PASSWORD='p@ss;"$x' \
  ...
  node Tests/Support/embedded/generate-embedded-network-config.mjs "$tmp/config.h"
...
if AXOLOTY_WIFI_SSID=ssid AXOLOTY_WIFI_PASSWORD=password \
  ...
  node Tests/Support/embedded/generate-embedded-network-config.mjs "$tmp/invalid.h" 2>/dev/null; then
