...
run_swift() {
    ...
    BUILD_DIR="$build" SPM_CACHE_DIR="${SPM_CACHE_DIR:-$HOME/.cache/coaty-swift/swift-6.4-linux}" \
    CONTAINER_ENV_VARS=ASAN_OPTIONS ASAN_OPTIONS="${ASAN_OPTIONS:-detect_leaks=0}" \
    "$root/.devcontainer/run.sh" "$@"
}
CONTAINER_RUNTIME=${CONTAINER_RUNTIME:-podman} IMAGE=${IMAGE:-axoloty-dev} \
BUILD_DIR="$build" SPM_CACHE_DIR="${SPM_CACHE_DIR:-$HOME/.cache/coaty-swift/swiftpm/swift-6.4-linux}" \
