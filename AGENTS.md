...
- **Swift 6 strict concurrency gotchas when importing Axoloty:** (1) C globals like `stdout` are not concurrency-safe — use `isatty(1)` instead of `fileno(stdout)`. (2) `JSONEncoder` on Linux escapes `/` as `\/` by default — prefer `JSONSerialization`-based assertions over raw string matching in tests.
...
- For embedded setup failures, run `make embedded-toolchain-doctor`. It is device-independent and validates the ESP-IDF runtime used by `make embedded-swift-build`. Device access and credentials are checked separately by the opt-in hardware targets.
...
- Offline release evidence vs fresh wire evidence are distinct: `make release-fixture-bundle` (Linux) or `axoloty-tool release fixture-bundle` (macOS) bundles only committed fixtures and declares `evidence.live: false`; fresh wire evidence comes only from the live reference-agent capture path `wire-live`. Checkpoint validation: `make checkpoint` (ordinary) or `make checkpoint-hardware` (requires device). Generated bundles under `.testing/`; only reviewed stable fixtures in source control.
