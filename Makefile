...
g1-bounded-runtime-device: image
	@CONTAINER_RUNTIME="$(CONTAINER_RUNTIME)" IMAGE="$(IMAGE)" \
	BUILD_DIR="$(BUILD_DIR)" SPM_CACHE_DIR="$(SPM_CACHE_DIR)" \
	CONTAINER_DEVICES="$(AXOLOTY_DEVICE)" \
	CONTAINER_ENV_VARS='EMBEDDED_DEVICE GIT_CONFIG_COUNT GIT_CONFIG_KEY_0 GIT_CONFIG_VALUE_0' \
	EMBEDDED_DEVICE="$(AXOLOTY_DEVICE)" GIT_CONFIG_COUNT=1 \
	GIT_CONFIG_KEY_0=safe.directory GIT_CONFIG_VALUE_0=/workspace \
	.devcontainer/run.sh Spikes/BoundedPortableRuntime/check-device.sh
